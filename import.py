    ...
    def avoid_empty_SN(self, num=40, lc_length_prepeak=-50, lc_length_postpeak=135):
        ...
        if all(elem in self.band for elem in self.filters):
            ...
            for i in range(len(self.filters)):
                self.t_duration = [j for j in self.t[i] if (j > (self.t_max + lc_length_prepeak)) and (j < (self.t_max + lc_length_postpeak))]
...
def main():
    ...
    filter_all = ['g', 'r', 'i'] # Selected band system
    ...
    for i, filename in tenumerate(filenames):
        with open(filename, encoding="utf-8") as f:
            ...
            filename_QC2 = QC(filename, filter_all, json_data).avoid_empty_SN(num=30)

            if (filename_QC1 and filename_QC2) is True: # Choosing SNe that are both SNIa and not empty

                LC_result = LC_Preprocess(filename, filter_all, json_data).lc_extractor(peak_alignment=True, LC_graph=True)
